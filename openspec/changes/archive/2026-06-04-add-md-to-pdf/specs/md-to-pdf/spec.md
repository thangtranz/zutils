...
The Markdown-to-PDF tool SHALL be a React component reachable from the zutils sidebar as a nav entry, rendered in the main content area like the other tools.
...
The tool SHALL provide a left-pane textarea for Markdown source and a control to load a `.md`/`.markdown` file into it, mirroring the ANSI Converter layout.
...
The tool SHALL render the Markdown source to HTML using `marked`, applying GitHub-style styling for headings, code, tables, blockquotes, links, and horizontal rules, displayed in the right-pane preview.
...
The tool SHALL render fenced ```mermaid``` blocks as diagrams in the preview. The custom renderer SHALL emit `<pre class="mermaid">` with the diagram source HTML-escaped, and `mermaid` SHALL be initialized with `startOnLoad: false` and run after the preview HTML is committed.
...
The tool SHALL follow the app theme from `ThemeContext`. Page styling SHALL use the existing CSS variables, and the Mermaid theme SHALL be `dark` when the app theme is dark and `default` otherwise.
...
The tool SHALL export the rendered document to PDF using the browser's native print-to-PDF. An "Export PDF" action SHALL trigger `window.print()`, and a print-scoped stylesheet SHALL print only the rendered preview (hiding sidebar, input pane, and toolbar) with A4 page size and document margins.
...
The tool SHALL operate entirely in the browser with no backend or `puppeteer` dependency, and SHALL ship with the existing GitHub-Pages build.
