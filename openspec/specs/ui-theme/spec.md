...
Provides a global dark/light theme system for the zutils app. A React context exposes the current theme and a toggle function to all components, with the selected theme persisted in `localStorage`. CSS custom properties (design tokens) drive all theme-variant colors so individual pages and components require no direct theme branching.
